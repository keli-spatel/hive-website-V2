...
export default function AnimatedLogoCloud({ logos, className, ...props }: LogoCloudProps) {
  ...
  return (
    ...
    >
      ...
      {/* Card 0 */}
      <LogoCard logo={slots[0]} className="relative border-r border-b border-black/5 bg-zinc-50">
        <PlusIcon className="-right-[12.5px] -bottom-[12.5px] absolute z-10 size-6 text-black/20" strokeWidth={1.5} />
      </LogoCard>

      {/* Card 1 */}
      <LogoCard logo={slots[1]} className="border-b border-black/5 md:border-r" />

      {/* Card 2 */}
      <LogoCard logo={slots[2]} className="relative border-r border-b border-black/5 md:bg-zinc-50">
        <PlusIcon className="-right-[12.5px] -bottom-[12.5px] absolute z-10 size-6 text-black/20" strokeWidth={1.5} />
        <PlusIcon className="-bottom-[12.5px] -left-[12.5px] absolute z-10 hidden size-6 md:block text-black/20" strokeWidth={1.5} />
      </LogoCard>

      {/* Card 3 */}
      <LogoCard logo={slots[3]} className="relative border-b border-black/5 bg-zinc-50 md:bg-white" />

      {/* Card 4 */}
      <LogoCard logo={slots[4]} className="relative border-r border-b border-black/5 bg-zinc-50 md:border-b-0 md:bg-white">
      ...
      {/* Card 5 */}
      <LogoCard logo={slots[5]} className="border-b border-black/5 bg-white md:border-r md:border-b-0 md:bg-zinc-50" />

      {/* Card 6 */}
      <LogoCard logo={slots[6]} className="border-r border-black/5" />

      {/* Card 7 */}
      <LogoCard logo={slots[7]} className="bg-zinc-50" />
...
type LogoCardProps = {
  logo: { src: string; animKey: number };
  className?: string;
  children?: React.ReactNode;
};

function LogoCard({ logo, className, children, ...props }: LogoCardProps & React.HTMLAttributes<HTMLDivElement>) {
  const [images, setImages] = useState([{ src: logo.src, key: logo.animKey }]);

  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setImages((prev) => {
      if (prev[prev.length - 1].src === logo.src) return prev;
      const next = [...prev, { src: logo.src, key: logo.animKey }];
      if (next.length > 2) return next.slice(next.length - 2);
      return next;
    });
  }, [logo.src, logo.animKey]);
